...
def add_new_student(new_student: Student, ):
    res = base_manager.execute("INSERT INTO app(surname, name) "
                               "VALUES (?, ?) "
...
def update_student(student_id: int, student: Student):
    res = base_manager.execute("UPDATE app SET surname = ?, name = ? WHERE id = ?",
                               args=(student.surname, student.name))
    return NewId(code=res['code'], id=student_id)


def delete_student(student_id: int):
    res = base_manager.execute("DELETE FROM app WHERE id = ?",
                               args=(student_id,))
    return NewId(code=res['code'], id=student_id)